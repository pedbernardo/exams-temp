...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/js/app.000.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony import */ var _components_table_multivalue__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./components/table-multivalue */ "./src/js/components/table-multivalue.js");
...
var specificData = {
  ppra: {
    productName: 'PPRA - PROGRAMA DE PREVENÇÃO DE RISCOS AMBIENTAIS QUALITATIVO (NR 09)',
    productId: '2.02.01.01.03.001.001',
    contract: 'CONT-14856-N8Y4F4',
    razaoSocial: 'O L INDUSTRIA DE PAPEIS LTDA',
    cnpj: '08855790000107',
    account: '410104030100',
    resultCenter: '30401020030001',
    unityId: "3958" // PRD: '3978' Feira de Santana HML: 3958

  },
  pcmso: {
    productName: 'PCMSO - PROGRAMA DE CONTROLE MÉDICO DE SAÚDE OCUPACIONAL (NR 07)',
    productId: '2.02.01.01.08.001.001',
    contract: 'CONT-11707-S0G4W8',
    razaoSocial: 'BAIXO SUL ARTEFATOS DE LATEX EIRELI',
    cnpj: '13190394000130',
    account: 'Saude Ocupacional',
    resultCenter: 'PCMSO (NR 07)',
    unityId: "3963" // PRD: '3987' Ilhéus HML: 3963
...
var defaultData = {
  ...
  endDate: '01/01/2022',
  nextReportDate: '01/05/2020',
  interval: 6,
  activeCycles: 'sim',
  reportType: 'semestral'
};
var readonly = ['cnpjCliente', 'cnpjEmpresaDeAtendimento', 'nomeProduto', 'intervaloMesesRelatorio', 'dataCicloRelatorio', 'novoCiclo', 'tipoRelatorio', 'codProduto', 'engenheiroTxt', 'responsavelPelaExecucaoDoServico'];
var h = {
  $inp: function $inp(id) {
    return document.querySelector("[xname=inp".concat(id, "]"));
  },
  addYears: function addYears(date, quantity) {
    var newDate = new Date(date);
    newDate.setFullYear(date.getFullYear() + quantity);
    return newDate;
  },
  addMonths: function addMonths(date, quantity) {
    var newDate = new Date(date);
...
function fillData(e) {
  e.preventDefault();
  var $contactsTable = document.querySelector('#tbl-contacts');
  var productType = this.dataset.product;

  var data = _objectSpread({}, defaultData, {}, specificData[productType]);

  Object(_components_table_multivalue__WEBPACK_IMPORTED_MODULE_7__["appendData"])($contactsTable, [{
    contatoNome: 'Alessany Marques',
    contatoEmail: 'analista.rh@olpapeis.teste.br'
  }, {
    contatoNome: 'Valdecir Roberto Bechel',
    contatoEmail: 'administrativo@olpapeis.teste.br'
  }, {
    contatoNome: 'Anderson S.',
    contatoEmail: 'auxiliar.qualidade@olpapeis.com.br'
  }, {
    contatoNome: 'Luciana Carvalho',
    contatoEmail: 'treinamento@olpapeis.com.br'
  }, {
    contatoNome: 'Pablo Josué',
    contatoEmail: 'seguranca@olpapeis.com.br'
  }]); // DADOS DO CONTATO

  h.$inp('numeroDaProposta').value = data.contract;
  h.$inp('razaoSocial').value = data.razaoSocial;
  h.$inp('cnpjCliente').value = data.cnpj;
  h.$inp('inicioContrato').value = data.startDate;
  h.$inp('validadeDaProposta').value = data.endDate; // DADOS DO PRODUTO

  h.$inp('nomeProduto').value = data.productName;
  h.$inp('empresaDeAtendimento').value = data.razaoSocial;
  h.$inp('cnpjEmpresaDeAtendimento').value = data.cnpj;
  h.$inp('contaContabil').value = data.account;
  h.$inp('centroResultado').value = data.resultCenter; // VARIÁVEIS

  h.$inp('intervaloMesesRelatorio').value = data.interval;
  h.$inp('dataCicloRelatorio').value = data.nextReportDate;
  h.$inp('novoCiclo').value = data.activeCycles;
  h.$inp('tipoRelatorio').value = data.reportType;
  h.$inp('codProduto').value = data.productId;
  h.$inp('codigoUnidadeExecutanteOrquestra').value = data.unityId;
}

function setEndDate() {
  var currDate = $(this).datepicker('getDate');
  var endDate = h.addYears(currDate, 2);
  h.$inp('validadeDaProposta').value = h.formatDate(endDate);
  setFirstCycle();
}

function setStartDate() {
  var currDate = $(this).datepicker('getDate');
  var startDate = h.addYears(currDate, -2);
  h.$inp('inicioContrato').value = h.formatDate(startDate);
  setFirstCycle();
}

function setFirstCycle() {
  var startDate = $(h.$inp('inicioContrato')).datepicker('getDate');
  var cycleDate = h.addMonths(startDate, 4);
  h.$inp('dataCicloRelatorio').value = h.formatDate(cycleDate);
}

function setReadOnly(fields) {
  fields.forEach(function (field) {
    var $field = h.$inp(field);
...
function mount() {
  $(h.$inp('inicioContrato')).on('change', setEndDate);
  $(h.$inp('validadeDaProposta')).on('change', setStartDate);
}
...
function init() {
  ...
  document.querySelectorAll('.js-fill').forEach(function (btn) {
    return btn.addEventListener('click', fillData);
  });
}
